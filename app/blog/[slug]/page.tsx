...
export default async function ArticlePage({ params }: Props) {
  ...
  return <>
    ...
    <main className="m-auto max-w-3xl px-4 py-16 lg:px-0"><div className="article-content text-lg leading-8 text-slate-700" dangerouslySetInnerHTML={{ __html: article.content || '' }} />{activeFaqs.length > 0 && <section className="mt-16"><h2 className="text-3xl font-bold text-slate-950">Frequently asked questions</h2><div className="mt-6 space-y-6">{activeFaqs.sort((a, b) => a.sort_order - b.sort_order).map((faq) => <div key={faq.question}><h3 className="text-xl font-bold text-slate-900">{faq.question}</h3><p className="mt-2 leading-7 text-slate-700">{faq.answer}</p></div>)}</div></section>}<section className="mt-16 rounded-2xl bg-slate-950 p-8 text-white"><h2 className="text-2xl font-bold">Ready to find a service professional?</h2><p className="mt-2 text-white/80">Explore the Bubbles app to see the options available in your area.</p><Link href="/" className="mt-5 inline-block font-semibold text-white underline">Explore Bubbles</Link></section></main>
    <Footer />
  </>
}
