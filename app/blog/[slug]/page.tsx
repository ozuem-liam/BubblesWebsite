...
import { articleCoverImageUrl, articleUrl, getPublishedArticle } from '@/lib/content'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: article.seo_title || article.title, description: article.meta_description || article.excerpt, type: 'article', images: articleCoverImageUrl(article.cover_image) ? [{ url: articleCoverImageUrl(article.cover_image)!, alt: article.cover_image_alt || article.title }] : undefined },
  }
}
...
export default async function ArticlePage({ params }: Props) {
  ...
  const schema = {
    ...
    publisher: { '@type': 'Organization', name: 'Bubbles', url: 'https://www.bubblesng.com' }, image: articleCoverImageUrl(article.cover_image),
  }
  ...
  return <>
    ...
    <main className="m-auto max-w-3xl px-4 py-16 lg:px-0">{articleCoverImageUrl(article.cover_image) && <div className="relative mb-12 aspect-[16/9] overflow-hidden rounded-2xl bg-slate-100"><Image src={articleCoverImageUrl(article.cover_image)!} alt={article.cover_image_alt || article.title} fill priority className="object-cover" sizes="(min-width: 1024px) 768px, 100vw" /></div>}<div className="article-content text-lg leading-8 text-slate-700" dangerouslySetInnerHTML={{ __html: article.content || '' }} />{activeFaqs.length > 0 && <section className="mt-16"><h2 className="text-3xl font-bold text-slate-950">Frequently asked questions</h2><div className="mt-6 space-y-6">{activeFaqs.sort((a, b) => a.sort_order - b.sort_order).map((faq) => <div key={faq.question}><h3 className="text-xl font-bold text-slate-900">{faq.question}</h3><p className="mt-2 leading-7 text-slate-700">{faq.answer}</p></div>)}</div></section>}<section className="mt-16 rounded-2xl bg-slate-950 p-8 text-white"><h2 className="text-2xl font-bold">Ready to find a service professional?</h2><p className="mt-2 text-white/80">Explore the Bubbles app to see the options available in your area.</p><Link href="/" className="mt-5 inline-block font-semibold text-white underline">Explore Bubbles</Link></section></main>
    <Footer />
  </>
}
