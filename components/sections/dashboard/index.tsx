...
export const Dashboard = () => {
  ...
  const handlePlaceSelected = async (place: any) => {
    ...
    if (!place.geometry || !place.address_components) {
      setAutoError(
        "Could not find details for this location. Please select a valid address from the list."
      );
      return;
    }
    ...
    if (street_address === city) {
      setAutoError(
        "Please select a more specific location, like a street or business."
      );
      return;
    }
    ...
    if (street_address && city && state) {
      ...
      setAutoError(
        "Incomplete address details. Please select a more specific location."
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (!googleReady || !showAddressModal || !autocompleteInputRef.current)
      return;
    ...
    <>
      ...
      <div className="bg-gray-50 min-h-screen">
        ...
        >
          ...
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 text-gray-500 ml-2 flex-shrink-0"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
                  ...
                  {addressLoading ? (
                    ...
                    <div className="space-y-1">
                      {addresses.map((addr) => (
                        <div
                          key={addr._id}
                          className={`p-3 rounded-lg flex items-center cursor-pointer hover:bg-gray-100 transition-colors ${
                            addr.is_active ? "bg-gray-100" : ""
                          }`}
                          onClick={() => {
  ...
  return (
    <section className="relative">
      ...
      <header className="mb-4 mt-2 flex flex-col items-start">
        <Text as="h2" style="text-gray-900 text-xl font-bold mb-1">
          Shop Our Latest Products
        </Text>
        <Text as="p" style="text-gray-600">
          Browse and shop the newest arrivals from Bubbles Store
        </Text>
      </header>
