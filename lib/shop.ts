...
export const shopService = {
  async getShops(token: string, page = 1, limit = 10): Promise<ShopListResponse> {
    return api.get<ShopListResponse>(`/customer/shops?page=${page}&limit=${limit}`, token);
  },

  async getShopById(id: string, token?: string): Promise<{ code: number; message: string; data: Shop }> {
    return api.get<{ code: number; message: string; data: Shop }>(`/customer/shops/${id}`, token);
  },
